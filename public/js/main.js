...
socket.on('join_room_response',function(payload){
  ...
  /* If somoneone joined then add a new row to the lobby table */
  ...
  /* If we have seen the person who just joined (something wack happened) */
  else{
...
socket.on('player_disconnected',function(payload){
  ...
  /* If somoneone left the room, then animate out all their content */
  var dom_elements = $('.socket_'+payload.socket_id);
...
/* Handle a response after ending an invite message to the server */
...
/* Send an uninvite to the server */
...
function makeInvitedButton(socket_id){
  var newHTML = '<button type=\'button\' class=\'btn btn-warning\'>Invited</button>';
  var newNode = $(newHTML);
  newNode.click(function(){
      uninvite(socket_id);
  });
...
socket.on('game_update',function(payload){
  ...
  if(socket.id == payload.game.player_white.socket){
    my_color = 'white';
      ...
      $('#my_color').html('<h3 id="my_color">You are the '+my_color+' player.</h3>');
      $('#my_color').append('<h4>It is '+payload.game.whose_turn+'\'s turn. Time since last play: <span id="elapsed"></span></h4>');
...
socket.on('play_token_response',function(payload){
  ...
  $('#game_over').append('<a href="lobby.html?username='+username+'" id="return_button" class="btn btn-warning btn-lrg active" role="button" aria-pressed="true">Return to the lobby</a>');
